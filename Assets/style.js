    ...
    citySearchEl.on("input", (event) => {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                const date = dayjs.unix(data.dt).format("YYYY-MM-DD HH:mm:ss");
                console.log(`The current date in ${data.city.name} is ${date}`);

                var cardEl = $(".card");
                cardEl.find(".card-title").text(data.city.name)//dayjs()?
                cardEl.find(".card-text-temp").text(`temp: ${data.list[0].main.temp}`); //convert from K to C. get current date.
